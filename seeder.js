	...
	function permissionServiceSeeder(callback) {
		User.findOne({
			...
			Promise.all([
				...
				const [manageUserService, manageProfileService, managePermissionService, manageServices, systemAdminPermission] = values;
				...
				const permission_services = [
					...
					{ permission_id: systemAdminPermission.id, service_id: manageServices.id },
				];
