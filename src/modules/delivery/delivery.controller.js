const DeliveryMan = require("./delivery.model.js");

// sign up functionality for delivery man.
const signupDeliveryMan = (req, res) => {
  const data = { ...req.body };
  // check email exsists or not.
  DeliveryMan.findOne({ where: { email: data.email } })
    .then((result) => {
      if (result) {
        res.send({
          message: `${result.dataValues.email} already exsists,choose different email.`,
        });
      } else {
